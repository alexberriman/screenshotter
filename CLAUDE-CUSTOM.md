...
This is a Node.js CLI tool for taking screenshots of websites. Key principles:

- **Executable CLI**: The main entry point is a CLI binary that uses Commander.js
- **Playwright**: Uses Playwright for browser automation and screenshots
...
src/
  ...
  screenshot.ts     # Core screenshot functionality
...
- Avoid generating code that mixes concerns (e.g. screenshot logic + CLI parsing)
- Suggest **domain-appropriate file names** (e.g., viewport-parser.ts, browser-manager.ts)
